...
const LOGO_BRAND_COLOR = "#1e3a8a"; // same as Tailwind text-blue-900

/**
 * Inline DontWait logo, force-colored to brand blue using CSS mask.
 * Requires: /public/images/dontwait.svg
 */
const InlineLogo: React.FC<{ className?: string; size?: "sm" | "lg" }> = ({
  className,
  size = "sm",
}) => {
  const height = size === "lg" ? 56 : 32; // px
  const width = size === "lg" ? 220 : 140; // px
        ...
        style={
          {
            ...
            width,
            height,
            // 👇 Fixed: same blue as the title (text-blue-900)
            backgroundColor: LOGO_BRAND_COLOR,
...
const HowItWorksSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => setActiveStep(3), 2500);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
      clearTimeout(t3);
