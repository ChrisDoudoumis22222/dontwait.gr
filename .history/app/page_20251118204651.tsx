...
// import { ProgressBar } from "@/components/progress-bar"; // ✅ removed
...
// ---------------------------------------------
// HEADER Component (fixed, with mobile menu)
// ---------------------------------------------
...
function Header({ onTrialOpen }: HeaderProps) {
  return (
    <Popover
      as="header"
      className="fixed top-0 inset-x-0 z-50 bg-gradient-to-r from-white via-blue-50 to-white border-b border-blue-100 shadow-md backdrop-blur-sm"
    >
      {({ open }) => (
        <>
          ...
          >
            <Popover.Panel className="md:hidden absolute inset-x-0 top-full bg-white/95 backdrop-blur-md shadow-lg border-t border-gray-200">
              <div className="container mx-auto px-4 pt-4 pb-4 space-y-4">
                {NAV_ITEMS.map(({ label, icon: Icon }) => (
                  <Popover.Button
                    key={label}
                    as={ScrollLink}
