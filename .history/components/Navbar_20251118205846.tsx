...
export const Navbar: React.FC<NavbarProps> = ({ onTrialOpen }) => {
  return (
    // Full-width position, but content inside is max-w
    <Popover className="fixed top-0 inset-x-0 z-50 flex justify-center">
      {/* ✅ Limited width, centered bar */}
      <div className="w-full max-w-6xl bg-gradient-to-r from-white via-blue-50 to-white border-b border-blue-100 shadow-md backdrop-blur-sm">
        ...
        {/* Mobile panel (same width limits) */}
        <Transition
          as="div"
        ...
        >
          <Popover.Panel className="md:hidden bg-white/90 backdrop-blur-md shadow-lg border-t border-gray-200">
            <div className="px-4 sm:px-6 lg:px-8 pt-4 pb-4 space-y-4">
              {NAV_ITEMS.map(({ label, icon: Icon }) => (
                <ScrollLink
                  key={label}
                  to={label}
