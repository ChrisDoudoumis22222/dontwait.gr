...
export const TrialModal: React.FC<TrialModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Extra safety: if "other" selected, ensure description is filled
    if (
      formData.companyType === "other" &&
      !formData.companyTypeOther.trim()
    ) {
      alert("Παρακαλώ περιγράψτε τον τύπο της επιχείρησής σας.");
      return;
    }

    setIsSubmitting(true);

    // If 'other' -> use companyTypeOther, else use the selected option
    const typeToStore =
      formData.companyType === "other"
        ? formData.companyTypeOther.trim()
      ...
      const { error } = await supabase.from(`Request Form`).insert([
        {
          ...
          Type: typeToStore, // store either selected type or custom description
              ...
              {formData.companyType === "other" && (
                <div>
                  ...
                  <motion.input
                    ...
                    required={formData.companyType === "other"}
                    disabled={isSubmitting}
