...
export const PlanSelectionForm: React.FC<PlanSelectionFormProps> = ({
  ...
  const getProgressValue = () => {
    // Base progress from completed previous steps
    const stepBase = (step - 1) / totalSteps; // 0, 1/3, 2/3
    const currentStepWeight = 1 / totalSteps; // each step ~33.3%

    let currentCompletion = 0;

    if (step === 1) {
      // Step 1: select package (0% or 100% of this slice)
      currentCompletion = formData.package ? 1 : 0;
    } else if (step === 2) {
      // Step 2: name, email, phone -> each 1/3 of this slice
      const fields = [formData.name, formData.email, formData.phone];
      const filled = fields.filter((v) => v && v.trim().length > 0).length;
      currentCompletion = filled / fields.length;
    } else if (step === 3) {
      // Step 3: when user reaches final step, treat as full slice (comment optional)
      return 100;
    }

    const progress = (stepBase + currentCompletion * currentStepWeight) * 100;
                        ...
                        {[1, 2, 3].map((s) => {
                          ...
                          return (
                            <div key={s} className="flex items-center">
                              ...
                              {s < 3 && (
                                <div className="h-[2px] w-7 sm:w-9 md:w-12 rounded-full bg-white/20 overflow-hidden mx-1">
                                  <motion.div
                                    initial={false}
                                    animate={{
                                      scaleX:
                      ...
                      <div className="w-full mt-1">
                        <div className="h-1.5 rounded-full bg-white/15 overflow-hidden">
                          <motion.div
                            ...
                            transition={{ duration: 0.25, ease: "easeOut" }}
                            className="h-full rounded-full bg-gradient-to-r from-emerald-300 via-sky-200 to-white"
                            style={{ originX: 0 }}
                          />
                        </div>
                        <div className="mt-1 flex items-center justify-between text-[10px] sm:text-[11px] text-white/80">
                          <span>Βήμα {step} από 3</span>
                          <span>{progressValue}% ολοκλήρωση</span>
                        </div>
                      </div>
                    </div>
                  </>
                ...
                <AnimatePresence mode="wait" custom={direction}>
                  ...
                  {step === 3 && (
                    ...
                    >
                      ...
                      <p className="text-xs sm:text-sm text-gray-500 mb-6">
                        Προσθέστε οποιαδήποτε πρόσθετη πληροφορία (προαιρετικό).
                      </p>
