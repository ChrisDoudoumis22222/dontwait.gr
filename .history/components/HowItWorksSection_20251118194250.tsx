...
const HowItWorksSection: React.FC = () => {
  const [activeStep, setActiveStep] = useState(0); // for left timeline animation
  const [imageStep, setImageStep] = useState(1); // for looping carousel

  // One-time timeline animation (1 → 2 → 3)
  useEffect(() => {
  ...
  // Infinite image carousel: 1 → 2 → 3 → 1 → ...
  useEffect(() => {
    const interval = setInterval(() => {
      setImageStep((prev) => (prev % 3) + 1);
    }, 7000); // slower change (7 seconds)
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-start gap-12 lg:gap-16 max-w-6xl mx-auto">
          ...
          <div className="w-full lg:flex-1">
            ...
            {/* Removed the explanatory paragraph under the image */}
          </div>
        </div>
      </div>
    </section>
